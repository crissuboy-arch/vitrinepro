...
export default function CatalogEditorPage() {
  ...
  const handleShare = async () => {
    ...
    try {
      await navigator.clipboard.writeText(`${process.env.NEXT_PUBLIC_CATALOG_URL || 'https://vitrine.vitriodigital.com'}/catalogo/${slug}`)
        ...
        {catalog.publico && catalog.slug && (
          <div style={{ display: "flex", alignItems: "center", gap: "10px", marginLeft: "auto" }}>
            <span style={{ fontSize: "12px", color: "#555", fontFamily: "monospace" }}>
              {`${process.env.NEXT_PUBLIC_CATALOG_URL || 'https://vitrine.vitriodigital.com'}/catalogo/${catalog.slug}`}
            </span>
            <button
              onClick={async () => {
                try {
                  await navigator.clipboard.writeText(`${process.env.NEXT_PUBLIC_CATALOG_URL || 'https://vitrine.vitriodigital.com'}/catalogo/${catalog.slug}`)
                  showToast("Link copiado!")
                } catch {
