...
function buildHtml(catalog: Catalog): string {
  ...
  <div class="page" style="background:${escapeHtml(cores.fundo)};">
    ...
    <div style="padding:48px 56px;">
      ...
      <h2 style="font-family:'Playfair Display',Georgia,serif;font-size:36px;font-weight:700;color:${escapeHtml(cores.titulos)};margin-bottom:16px;">${escapeHtml(p.titulo)}</h2>
      ${p.descricao ? `<p style="font-size:15px;color:#333;opacity:0.75;line-height:1.7;margin-bottom:24px;">${escapeHtml(p.descricao)}</p>` : ""}
      ${p.preco ? `<div style="font-family:'Playfair Display',Georgia,serif;font-size:42px;font-weight:700;color:${escapeHtml(cores.precos)};">${escapeHtml(p.preco)}</div>` : ""}
    </div>
...
<body>
  ...
  <div class="page" style="background:${escapeHtml(cores.secundaria)};">
    ...
    <div style="position:relative;z-index:1;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:60px;text-align:center;">
      ...
      <h1 style="font-family:'Playfair Display',Georgia,serif;font-size:56px;font-weight:700;color:${escapeHtml(cores.titulos)};line-height:1.1;margin-bottom:16px;">${escapeHtml(capa.nome)}</h1>
  ...
  <div class="page" style="background:${escapeHtml(cores.secundaria)};display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:60px;">
    <div style="font-family:'Playfair Display',Georgia,serif;font-size:32px;font-weight:700;color:${escapeHtml(cores.titulos)};margin-bottom:12px;">${escapeHtml(capa.nome)}</div>
